...
def _register_bpu_compile_backend() -> None:
    """Register torch.compile(backend="bpu") on an BPU build.

    The RDK BPU's BPU executes whole compiled graphs (a .hbm produced by
    hbdk4), not individual operators, so it has no per-op kernels: eager ops
    reach cpu_fallback and all acceleration comes through this backend. That is
    the opposite of every other platform here, where the compile path is
    incidental and the kernels do the work.
    ...
    if _build_accelerator() != "bpu":
        return
    try:
        from torch_fl.backends import bpu
