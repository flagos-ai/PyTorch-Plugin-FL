...
at::Tensor {kernel}(const at::Tensor& self, const at::Tensor& other) {{
  namespace ascend = at::native::flagos::ascend;
{scalar_fastpath}""" + _BINARY_PROLOGUE_BODY + """\
  auto out = ascend::OpPreparation::apply_tensor_without_format(
      out_shape, self.options());
...
NO_ACLNN_CATEGORIES = {"zeros", "ones", "scalar_tensor", "ones_like", "zeros_like",
                       "empty_like", "full", "full_like", "new_ones"}
...
def main():
        ...
        if exec_cache and cat in CACHED_CATEGORIES:
            ...
            if cat in ("binary", "binary_alpha"):
                ...
                if exec_cache and op in SCALAR_VARIANT:
                    ...
                    acl_s = "aclnn" + sname
                    if syms is None or (acl_s in syms and acl_s + "GetWorkspaceSize" in syms):
                        tmpl = _SCALAR_FASTPATH_ALPHA if kind == "alpha" else _SCALAR_FASTPATH_NOALPHA
    ...
    if exec_cache:
        print(f"    [exec-cache] ON for {len(cached_ops)} op(s): {', '.join(cached_ops)}")
        if scalar_fastpath_ops:
            print(f"    [scalar-fastpath] {len(scalar_fastpath_ops)} op(s): {', '.join(scalar_fastpath_ops)}")
    else:
        print(f"    [exec-cache] OFF (FLAGOS_EXEC_CACHE=0)")
    by_cat = {}
