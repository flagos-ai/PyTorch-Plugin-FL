...
__global__ void SiluContigKernel(
    int64_t n,
  ...
  out[idx] = static_cast<scalar_t>(x / (opmath_t(1) + ::exp(-x)));
}

template <typename scalar_t, typename opmath_t>
void LaunchSiluIter(at::TensorIteratorBase& iter) {
...
inline at::Tensor SiluKernelMetax(const at::Tensor& self) {
    ...
    for (auto& sub_iter : iter.with_32bit_indexing()) {
      AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND2(
          ...
          [&]() {
            ...
            LaunchSiluIter<scalar_t, opmath_t>(sub_iter);
          });
    }
    return iter.output();
  }
  ...
  TORCH_CHECK(
      iter.is_contiguous(),
      "MetaX silu requires contiguous flagos tensors");
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND2(
      ...
      [&]() {
        using opmath_t = at::opmath_type<scalar_t>;
        LaunchSiluIter<scalar_t, opmath_t>(iter);
      });
