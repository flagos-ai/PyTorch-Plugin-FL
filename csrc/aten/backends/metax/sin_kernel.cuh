...
__global__ void SinContigKernel(
    int64_t n,
  ...
  out[idx] = static_cast<scalar_t>(
      ::sin(static_cast<opmath_t>(self[idx])));
}

template <typename scalar_t, typename opmath_t>
void LaunchSinIter(at::TensorIteratorBase& iter) {
...
inline at::Tensor SinKernelMetax(const at::Tensor& self) {
    ...
    for (auto& sub_iter : iter.with_32bit_indexing()) {
      AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND2(
          ...
          [&]() {
            ...
            LaunchSinIter<scalar_t, opmath_t>(sub_iter);
          });
    }
    return iter.output();
  }
  ...
  TORCH_CHECK(
      iter.is_contiguous(),
      "MetaX sin requires contiguous flagos tensors");
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND2(
      ...
      [&]() {
        using opmath_t = at::opmath_type<scalar_t>;
        LaunchSinIter<scalar_t, opmath_t>(iter);
      });
