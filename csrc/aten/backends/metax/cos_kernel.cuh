...
__global__ void CosContigKernel(
    int64_t n,
  ...
  out[idx] = static_cast<scalar_t>(::cos(static_cast<opmath_t>(self[idx])));
}

template <typename scalar_t, typename opmath_t>
void LaunchCosIter(at::TensorIteratorBase& iter) {
...
inline at::Tensor CosKernelMetax(const at::Tensor& self) {
    ...
    for (auto& sub_iter : iter.with_32bit_indexing()) {
      AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND2(
          ...
          [&]() {
            ...
            LaunchCosIter<scalar_t, opmath_t>(sub_iter);
          });
    }
    return iter.output();
  }
  ...
  TORCH_CHECK(
      iter.is_contiguous(),
      "MetaX cos requires contiguous flagos tensors");
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND2(
      ...
      [&]() {
        using opmath_t = at::opmath_type<scalar_t>;
        LaunchCosIter<scalar_t, opmath_t>(iter);
      });
