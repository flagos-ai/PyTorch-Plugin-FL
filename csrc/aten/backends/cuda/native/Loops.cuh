...
//
// Adopted from pytorch/aten/src/ATen/native/cuda/Loops.cuh
// Modified: relaxed device assertions to accept PrivateUse1 (flagos) tensors.
...
void gpu_kernel_nocast(TensorIteratorBase& iter, const func_t& f, bool check_cast = true) {
  ...
  for (int arg = 0; arg < iter.ntensors(); arg++) {
    TORCH_INTERNAL_ASSERT(
      iter.device(arg).is_cuda() || iter.device(arg).is_privateuseone(),
      "argument ", arg, ": expected a CUDA/PrivateUse1 device but found ", iter.device(arg));
  }
